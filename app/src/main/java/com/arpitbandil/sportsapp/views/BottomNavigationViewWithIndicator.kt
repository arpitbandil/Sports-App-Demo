package com.arpitbandil.sportsapp.views
...
class BottomNavigationViewWithIndicator : BottomNavigationView,
    ...
    private val indicatorShadow = RectF()
    ...
    override fun dispatchDraw(canvas: Canvas) {
        ...
        if (isLaidOut) {
            ...
            shadowPaint.shader = LinearGradient(
                indicatorShadow.centerX(),
                indicatorShadow.top,
                indicatorShadow.centerX(),
                indicatorShadow.bottom,
                Color.parseColor("#80EC6437"),
                Color.TRANSPARENT,
                Shader.TileMode.DECAL
            )
            canvas.drawRect(indicatorShadow, shadowPaint)
    ...
    private fun onItemSelected(itemId: Int, animate: Boolean = true) {
        ...
        animator = ValueAnimator.ofFloat(MAX_SCALE, DEFAULT_SCALE, MAX_SCALE).apply {
            addUpdateListener {
                ...
                val left = distanceTravelled - indicatorWidth
                val right = distanceTravelled + indicatorWidth
                val top = 0f
                val bottom = defaultSize

                indicator.set(left, top, right, bottom)
                indicatorShadow.set(left, top, right, height.toFloat())
                invalidate()
            }
